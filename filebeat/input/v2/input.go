...
// MetricsID is the ID used for metrics, it's empty if the context ID was
// generated, or it's the context ID with any '.' replaced by '_'.
...
func (c *Context) MetricRegistry() *monitoring.Registry {
	...
	if c.monitoringRegistry == nil {
		reg := c.Agent.MonitoringRegistry().GetRegistry(c.MetricsID())
		if reg == nil {
			reg = c.Agent.MonitoringRegistry().NewRegistry(c.MetricsID())
		}

		c.monitoringRegistry = reg
		c.monitoringRegistryCancel = func() {
			c.Agent.MonitoringRegistry().Remove(c.MetricsID())
		}
	}
