...
func newFileWatcher(logger *logp.Logger, paths []string, ns *conf.Namespace, gzipAllowed bool) (loginp.FSWatcher, error) {
	var config *conf.C
	if ns == nil {
	...
	return newScannerWatcher(logger, paths, config, gzipAllowed)
}

func newScannerWatcher(logger *logp.Logger, paths []string, c *conf.C, gzipAllowed bool) (loginp.FSWatcher, error) {
